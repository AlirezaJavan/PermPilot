...
<p align="center">
  <img src="screenshots/sampleApp1.png" width="30%" />
  <img src="screenshots/sampleApp2.png" width="30%" />
  <img src="screenshots/sampleApp3.png" width="30%" />
</p>
...
### 1. Platform Setup
Declare your permissions in the platform-specific manifests. PermPilot reports `ConfigurationError` at runtime if these are missing.
- **Android**: Add `<uses-permission>` to `AndroidManifest.xml`.
- **iOS**: Add `NS*UsageDescription` keys to `Info.plist`.

See the [Permission Matrix](docs/permission-matrix.md) for the exact keys needed for each permission.

### 2. Implementation
The easiest way to request a permission is using the `PermissionGate` in your Compose UI.
...
fun CameraScreen() {
    ...
    // 2. Wrap your content in a Gate
    PermissionGate(
        permission = Permission.Camera,
        controller = controller
    ) { state ->
        // 3. Handle the state (Gate handles Rationale/Settings dialogs automatically)
        when (state) {
            PermissionState.Granted -> CameraPreview()
            is PermissionState.Limited -> CameraPreview(isLimited = true)
            else -> Box(Modifier.fillMaxSize()) { Text("Camera access required") }
        }
    }
}
```

For non-UI logic or custom flows, use the controller directly:
...
Button(onClick = {
    scope.launch {
        val result = controller.request(Permission.Camera)
