...
Project instructions for Claude Code when working in this repository. This file is the operational, day-to-day reference — architecture rationale and the platform gotchas live in the "Hard rules learned the hard way" section below, the permission matrix in `docs/permission-matrix.md`, and the forward-looking backlog (new permissions + features to add, with per-step done markers) in `docs/ROADMAP.md`.
...
9. **New permission checklist**: add to `Permission.kt` → Android manifest mapping (`toManifestPermissions()`) + real `check*State()` if `Special` → iOS native call + status mapping + `IosInfoPlistRequirements.kt` entry → sample app wiring → sample `AndroidManifest.xml` `<uses-permission>` → `docs/permission-matrix.md` row → `./gradlew :permpilot-core:apiDump` (public API surface changed).
...
11. **Any new `checkState()`/`refreshAll()` branch must stay side-effect-safe when called repeatedly** — iOS's `Permission.Notifications` and Android's `Permission.Health` both have no synchronous status getter, so their `checkState()` branches return a placeholder (`NotDetermined`/`Denied`) and refresh the real value asynchronously as a side effect. Calling that branch from a loop (e.g. `refreshAll()`) will flash an already-`Granted` state back to the placeholder on every call — concretely, this bit Android Health: returning from the Health Connect grant screen triggers `ON_RESUME` → `refreshAll()` in the same instant the launcher callback publishes `Granted`, and the generic loop's placeholder overwrote it a moment later. `refreshAll()` must special-case any such permission to call its async refresh helper directly instead of going through the generic placeholder-returning `checkState()` path.
12. **Health Connect needs manifest entries beyond `<uses-permission>` to even appear in its app list** — a consumer app that only declares the `android.permission.health.*` permissions is invisible in Health Connect's "Apps" screen. Two more manifest pieces are required: an `Activity` handling `androidx.health.ACTION_SHOW_PERMISSIONS_RATIONALE` (pre-Android 14, APK Health Connect) **and** an `activity-alias` handling `android.intent.action.VIEW_PERMISSION_USAGE` + category `android.intent.category.HEALTH_PERMISSIONS`, permission `android.permission.health.START_ONBOARDING` (Android 14+, platform Health Connect — most current devices need this one). Both can target the same host `Activity`. See `sample/androidApp/src/main/AndroidManifest.xml` for the reference declaration, and `docs/health-design.md` for the consumer-facing writeup.
...
- `permpilot-core`, `permpilot-compose`, and `permpilot-history` all publish via `com.vanniktech.maven.publish` (`KotlinMultiplatform` variant, Dokka javadoc, GPG signing) to Maven Central under `io.github.alirezajavan`; coordinates and POM metadata live in root `gradle.properties`, except `permpilot-history`'s `version`, which it sets explicitly in its own `build.gradle.kts` (independently versioned from the other two, on purpose). `sample/*` is never published.
...
- Enable `.githooks/pre-commit` locally with `git config core.hooksPath .githooks` — it's the fast subset (Spotless/ktlint `spotlessApply` + `spotlessCheck`, then `permpilot-core` JVM tests + iOS frontend type-check), not the full `./gradlew build` CI runs.
- **Code formatting is Spotless (ktlint) configured once at the repo root `build.gradle.kts`** over all modules' `**/*.kt`/`**/*.gradle.kts`. Run `./gradlew spotlessApply` to format, `spotlessCheck` to verify (the pre-commit hook does both). Project conventions live in `.editorconfig`: `@Composable` functions are exempt from ktlint's `function-naming` rule (they're intentionally PascalCase); the one non-`@Composable` PascalCase entry point (`MainViewController`, an iOS Swift-called entry point) carries an explicit `@Suppress("ktlint:standard:function-naming")`. Don't reintroduce wildcard imports — ktlint's `no-wildcard-imports` will fail the build.
...
## Current gaps (see `docs/ROADMAP.md` for the full backlog and ordering)

`sample/iosApp`'s actual `.xcodeproj` (needs a Mac to create/verify — everything else it needs is staged, see `docs/ROADMAP.md` C3) and the manual QA matrix across real API levels / iOS versions (`docs/ROADMAP.md` C4). `permpilot-history`, `docs/permission-matrix.md`, and binary-compatibility-validator are all done — don't assume they're still gaps. New permissions and library features to build next are catalogued in `docs/ROADMAP.md` (Sections A/B).

**Fixed** (concurrency hardening, resolved 2026-07-02): overlapping `request()`/`requestAll()` calls for the same permission used to be able to hang the earlier caller forever — iOS's `locationContinuation`/`bluetoothContinuation` fields got overwritten by a second in-flight request, and Android's `multiRequestFlow` (`extraBufferCapacity = 1`) could silently drop a `tryEmit` when the buffer was already full. Both platforms now serialize `request()`/`requestAll()` through a per-controller `Mutex` (`requestMutex`). Keep this in mind if you touch either controller's request path: `requestAll()` must keep routing through an internal lock-free/`requestLocked()` helper instead of calling the public `request()` recursively, since `Mutex` isn't reentrant.

**Fixed** (Android Health Connect grant flashing back to Denied, resolved 2026-07-09): `refreshAll()` didn't special-case `Permission.Health` (see rule 11) — returning from the Health Connect permission screen fires `ON_RESUME` right as the launcher callback publishes `Granted`, and the generic `refreshAll()` loop's `checkHealthState()` placeholder overwrote it with `Denied(canRequestAgain = true)` before the async correction landed. Also fixed: the sample's `AndroidManifest.xml` was missing the manifest pieces rule 12 describes, so the sample app never appeared in Health Connect's app list at all on API 34+ (most current devices) — unrelated to the flash bug, but discovered investigating the same report.
