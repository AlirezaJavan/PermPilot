...
`sample/iosApp` Xcode project (needs a Mac), `permpilot-history`, `docs/permission-matrix.md`, binary-compatibility-validator. Don't assume any of these exist yet.

**Known, deliberately-deferred bug** (PLAN.md §9.1): overlapping `request()`/`requestAll()` calls for the same permission can hang the earlier caller forever — iOS's `locationContinuation`/`bluetoothContinuation` fields get overwritten by a second in-flight request, and Android's `multiRequestFlow` (`extraBufferCapacity = 1`) silently drops a `tryEmit` when the buffer's already full. Needs real request serialization (a `Mutex` or pending-request queue), not a quick patch — don't attempt a one-line fix without designing the queuing/cancellation semantics first.
