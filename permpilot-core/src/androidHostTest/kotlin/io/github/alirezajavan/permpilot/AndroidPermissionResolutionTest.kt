 ...
 * and the coarse-location-misreported-as-Denied bug found in this repo's own audit (see CLAUDE.md
 * rules 1 and 10).
 */
class AndroidPermissionResolutionTest {
    ...
    fun `fine granted resolves to Granted regardless of coarse`() {
        val state =
            resolveForegroundLocationGrantResult(
                results =
                    mapOf(
    ...
    fun `API 34 full media grant resolves to Granted`() {
        val state =
            resolvePhotoLibraryGrantResult(
                sdkInt = 34,
                results =
                    mapOf(
