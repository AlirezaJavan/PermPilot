...
class FakePermissionControllerTest {
    ...
    @Test
    fun `deny-then-retry-and-grant is a realistic two-request flow`() = runTest {
    ...
    @Test
    fun `permanently denied never resolves without an explicit setState -- mirrors real Settings-only recovery`() = runTest {
        val controller = FakePermissionController()
    ...
    @Test
    fun `requestAll resolves each permission independently and records every call`() = runTest {
        val controller = FakePermissionController()
        controller.setState(Permission.Camera, PermissionState.Granted)
